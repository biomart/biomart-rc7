...
public class UpdateMartModel {
	...
	public Map<String, List<String>> getDataBaseInfo(Dataset ds)
		...
		if (dlink.getDataLinkType() == DataLinkType.SOURCE
			...
			String dsName = index > 0 ? tmpDatasetName.substring(0, index)
					: tmpDatasetName;
	...
	private Map<String, List<String>> getDbInfoFromUrl(Dataset ds) {
		...
		/*
		 * JdbcLinkObject linkObject = new
		 * JdbcLinkObject(dlinkInfo.getJdbcLinkObject().getJdbcUrl()+schemaName,
		 * schemaName, schemaName,dlinkInfo.getJdbcLinkObject().getUserName(),
		 * dlinkInfo.getJdbcLinkObject().getPassword(),
		 * dlinkInfo.getJdbcLinkObject().getJdbcType(),
		...
		List<DatasetFromUrl> tmpDsList = McGuiUtils.INSTANCE
				.getDatasetsFromURL(fullhost + ":" + port + path, modifiedName);
	...
	public void updateDatasetTables(Mart mart, Dataset ds,
			Map<String, List<String>> newTables,
			Set<DatasetTable> newInMartTable, Set<DatasetTable> newInDsTable,
			Set<DatasetColumn> newInMartColumn,
			Set<DatasetColumn> newInDsColumn,
			Set<DatasetTable> dropInMartTable, Set<DatasetTable> dropInDsTable,
			Set<DatasetColumn> dropInMartColumn,
			Set<DatasetColumn> dropInDsColumn, boolean isNewForDataset) {
		...
		for (Map.Entry<String, List<String>> entry : newTables.entrySet()) {
			...
			if (!found) {
				...
				DatasetTable newDst = new DatasetTable(mart, entry.getKey(),
				...
				for (String newDscStr : entry.getValue()) {
					DatasetColumn existingCol = oldDst
							.getColumnByName(newDscStr);
					if (existingCol != null) {
						...
						// is it a bool column?
						DatasetColumn newColumn = new DatasetColumn(oldDst,
								newDscStr);
				...
				for (DatasetColumn dsc : droppedColumns) {
					if (dsc.inPartition(datasetName)) {
						this.dropColumnFromDataset(mart, dsc, datasetName,
								dropInMartColumn, dropInDsColumn);
					}
				}
			}// end of else
		}
		...
		for (DatasetTable droppedDst : droppedList) {
			...
			for (Column droppedDsc : droppedDst.getColumnList(datasetName)) {
				this.dropColumnFromDataset(mart, (DatasetColumn) droppedDsc,
						datasetName, dropInMartColumn, dropInDsColumn);
			}
		}

	}

	private void dropColumnFromDataset(Mart mart, DatasetColumn column,
			String datasetName, Set<DatasetColumn> dropInMartColumn,
			Set<DatasetColumn> dropInDsColumn) {
	...
	public boolean updateOptions(Mart mart, List<Dataset> dslist)
			throws MartBuilderException {
		// update options for all configs, datasets

		for (Config config : mart.getConfigList()) {

			List<Filter> allFilter = config.getFilters(new ArrayList<String>(),
					true, true);
			for (Filter filter : allFilter) {
				if (filter.hasDropDown()
						&& filter.getObjectStatus() == ValidationStatus.VALID) {
		...
		for (Dataset ds : mart.getDatasetList()) {
			...
			if (dlink.getDataLinkType() == DataLinkType.SOURCE
				...
				if (dlink.getDataLinkType() == DataLinkType.TARGET) {
					...
					try {
						MartController.getInstance()
								.updateDatasetFromSource(ds);
	...
	private void updateDsFromUrl8(Dataset ds) {
		...
		List<MartPointer> mpList = mart.getMartRegistry().getPortal()
				.getRootGuiContainer().getMartPointerListforMart(mart);
		if (mpList.isEmpty()) {
			return;
		}
		PartitionTable mainPT = mart.getSchemaPartitionTable();
		...
		String userName = mainPT.getValue(row, PartitionUtils.USERNAME);
		String password = mainPT.getValue(row, PartitionUtils.PASSWORD);
		String urlStr = fullhost
				+ (McUtils.isStringEmpty(port) ? "" : ":" + port)
				+ "/martservice/xml/configs/" + mpList.get(0).getName();
	...
	private void updateDatasetForDB(Dataset ds) throws MartBuilderException {
		...
		if (mart.getMainTable() != null
				&& mart.getMainTable().hasSubPartition()) {
		...
		if (tables.isEmpty()) {// skip the url for now {
			DataLinkInfo dlink = ds.getDataLinkInfoNonFlip();
			if (!(dlink.getDataLinkType() == DataLinkType.SOURCE || dlink
					.getDataLinkType() == DataLinkType.TARGET)) {
				throw new MartBuilderException(
						"database naming convention error");
			}
		}

		this.updateDatasetTables(mart, ds, tables, newInMartTable,
				newInDsTable, newInMartColumn, newInDsColumn, dropInMartTable,
				dropInDsTable, dropInMartColumn, dropInDsColumn, false);
		// update configs
		{
			Config config = mart.getMasterConfig();
			Container newAC = config.getContainerByName(Resources
					.get("NEWATTRIBUTE"));
			if (newAC == null) {
			...
			Container newFC = config.getContainerByName(Resources
					.get("NEWFILTER"));
			if (newFC == null) {
			...
			for (DatasetTable newDst : newInMartTable) {
				for (Column column : newDst.getColumnList()) {
					...
					if (column.getName().endsWith(Resources.get("keySuffix"))) {
						...
						List<DatasetTable> mainDstList = newDst.getMart()
								.getOrderedMainTableList();
						for (DatasetTable mainDst : mainDstList) {
							if (mainDst.getPrimaryKey().getColumns().iterator()
									.next().getName().equals(column.getName())) {
								// create relation
								PrimaryKey pk = mainDst.getPrimaryKey();
								if (!Relation.isRelationExist(pk, fk)) {
									try {
										new RelationTarget(pk, fk,
												Cardinality.MANY_A);
					...
					this.updateConfigForNewColumn(config,
							(DatasetColumn) column, newAC, newFC);
				}
			}
	...
	private void updateDsWith2SchemaPartitions(Dataset ds)
			throws MartBuilderException {
		Mart mart = ds.getParentMart();
		...
		McSQL mcsql = new McSQL();
		// get all database info
		Map<String, List<String>> tables = mcsql.getMartTablesInfo(ds
				.getDataLinkInfoNonFlip().getJdbcLinkObject(), dsName);
		if (tables.isEmpty()) {// skip the url for now {
	...
	private void updateConfigForNewColumn(Config config, DatasetColumn newDsc,
		...
		if (foundAttContainer != null) {
			...
			foundAttContainer = new Container(tableName
					+ Resources.get("ATTRIBUTESUFFIX"));
			newAC.addContainer(foundAttContainer);
			newA = this.createNaiveAttribute(config, foundAttContainer, newDsc);
		}

		Container foundFilterContainer = this.findContainer(config, newAC,
				newFC, tableName + Resources.get("FILTERSUFFIX"));
		if (foundFilterContainer != null) {
			this.createNaiveFilter(config, foundFilterContainer, newA);
		} else {
			foundFilterContainer = new Container(tableName
					+ Resources.get("FILTERSUFFIX"));
	...
	private Attribute createNaiveAttribute(Config mart, Container container,
			DatasetColumn dsc) {
	...
	private Filter createNaiveFilter(Config mart, Container container,
			Attribute attribute) {
		// check name
		String name = McGuiUtils.INSTANCE.getUniqueFilterName(mart,
				attribute.getName());
	 ...
	 */
	private Container findContainer(Config config,
			Container attributeContainer, Container filterContainer, String name) {
	...
	public boolean updateDataset(Dataset ds, boolean updateOption)
			throws MartBuilderException {
	...
	public boolean updateDatasets(List<Dataset> dslist, boolean updateOption)
			throws MartBuilderException {
		boolean checkOrphanColumn = false;
	...
	private void updateDatasetForSourceDB(Dataset ds)
			throws MartBuilderException {
