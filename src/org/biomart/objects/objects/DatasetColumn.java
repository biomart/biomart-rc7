...
public class DatasetColumn extends Column {
	...
	public int hashCode() {
		...
		hash = MartConfiguratorConstants.HASH_SEED2 * hash + (null==this.getPropertyValue(XMLElements.INTERNALNAME)? 
				0 : this.getPropertyValue(XMLElements.INTERNALNAME).toLowerCase().hashCode());	// Must specify toLowerCase here	TODO move to MCO?
		return hash;
	}
	...
	public org.jdom.Element generateXml() {
		...
		if(!this.getRange().isEmpty()) {
			...
			String ds = this.getDatasetTable().getMart().getSchemaPartitionTable().getValue(0, PartitionUtils.DATASETNAME);
	...
	public List<Attribute> getReferences() {
		...
		for(Config con: mart.getConfigList()) {
			for(Attribute attribute: con.getAttributes(new ArrayList<String>(), true, true)) {
				if(attribute.getDataSetColumn()!=null && attribute.getDataSetColumn().getName().equals(this.getName())
...
//			if(attribute.getDataSetColumn()!=null && attribute.getDatasetTable() !=null && attribute.getDataSetColumn().getName().equals(this.getName())
	//				&& tableName.equals(attribute.getDatasetTable().getName()))
